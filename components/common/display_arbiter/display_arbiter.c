...
bool display_arbiter_owner_is_valid(display_arbiter_owner_t owner)
{
    return owner == DISPLAY_ARBITER_OWNER_LUA ||
           owner == DISPLAY_ARBITER_OWNER_EMOTE ||
           owner == DISPLAY_ARBITER_OWNER_USB_MSC;
}

bool display_arbiter_owner_uses_emote_gfx(display_arbiter_owner_t owner)
{
    return owner == DISPLAY_ARBITER_OWNER_EMOTE;
}
...
{
    (void)owner;
    return s_state.lua_depth > 0 ? DISPLAY_ARBITER_OWNER_LUA : DISPLAY_ARBITER_OWNER_EMOTE;
}
...
esp_err_t display_arbiter_acquire(display_arbiter_owner_t owner)
{
    ...
    ESP_GOTO_ON_FALSE(s_state.owner != DISPLAY_ARBITER_OWNER_USB_MSC || owner == DISPLAY_ARBITER_OWNER_USB_MSC,
                      ESP_ERR_INVALID_STATE, fail, TAG, "display locked by USB MSC");
