...
static esp_err_t cap_im_wechat_send_message_json(cJSON *msg_root)
{
    ...
    if (response.buf != NULL && response.buf[0] != '\0') {
        resp_root = cJSON_Parse(response.buf);
        if (!resp_root || !cap_im_wechat_json_has_success_code(resp_root)) {
            response_text = response.buf;
            ESP_LOGW(TAG, "wechat sendmessage returned error body=%s", response_text);
            cJSON_Delete(resp_root);
            cap_im_wechat_resp_cleanup(&response);
            return ESP_FAIL;
        }
        ESP_LOGI(TAG, "wechat sendmessage response=%s", response.buf);
        cJSON_Delete(resp_root);
    }
    cap_im_wechat_resp_cleanup(&response);
    return ESP_OK;
...
esp_err_t cap_im_wechat_qr_login_get_status(cap_im_wechat_qr_login_status_t *out_status)
{
    ...
    strlcpy(out_status->qrcode, s_wechat.qr.qrcode, sizeof(out_status->qrcode));
