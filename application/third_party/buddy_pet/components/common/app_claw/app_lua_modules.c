...
static bool app_lua_module_enabled_by_default(const app_lua_module_entry_t *entry)
{
    if (!entry || !entry->module_id) {
        return false;
    }

    return strcmp(entry->module_id, "camera") != 0 &&
           strcmp(entry->module_id, "vision") != 0;
}
                                          ...
                                          bool *selected_map)
{
    ...
    if (app_lua_modules_config_empty(configured_modules)) {
        for (i = 0; i < entry_count; i++) {
            selected_map[i] = app_lua_module_enabled_by_default(&entries[i]);
        }
        return ESP_OK;
    }
