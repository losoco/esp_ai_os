                                     ...
                                     const app_claw_storage_paths_t *paths)
{
    ...
    ESP_RETURN_ON_ERROR(app_lua_modules_register(config, paths->fatfs_base_path),
                        TAG, "Failed to register Lua modules");
    if (config->extra_lua_modules_cb) {
        ESP_RETURN_ON_ERROR(config->extra_lua_modules_cb(config, paths),
                            TAG, "Failed to register app Lua modules");
    }
    return ESP_OK;
}
