...
static esp_err_t config_post_handler(httpd_req_t *req)
{
    ...
    for (size_t i = 0; i < CONFIG_FIELD_COUNT; i++) {
        ...
        if (strcmp(field->name, "wechat_token") == 0 && item->valuestring[0] != '\0') {
            saved_wechat_token = true;
        }
        applied_count++;
    }
    ...
    strlcpy(config->llm_supports_vision, "false", sizeof(config->llm_supports_vision));
    ...
    if (saved_wechat_token && ctx->services.wechat_login_mark_persisted) {
        esp_err_t mark_err = ctx->services.wechat_login_mark_persisted();
        if (mark_err != ESP_OK) {
            ESP_LOGW(TAG, "Failed to mark WeChat login persisted: %s", esp_err_to_name(mark_err));
        }
    }
    free(config);
