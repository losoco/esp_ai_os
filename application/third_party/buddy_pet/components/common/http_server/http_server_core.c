...
esp_err_t http_server_captive_404_handler(httpd_req_t *req, httpd_err_code_t error)
{
    ...
    if (s_ctx.services.load_config && s_ctx.services.load_config(&config) == ESP_OK) {
        wifi_configured = config.wifi_ssid[0] != '\0';
    }

    const char *ap_ip = (status.ap_ip && status.ap_ip[0]) ? status.ap_ip : "192.168.4.1";
    char location[48];
    snprintf(location, sizeof(location), "http://%s/%s", ap_ip, wifi_configured ? "" : "#start");
...
esp_err_t http_server_start(void)
{
    ...
    ESP_RETURN_ON_ERROR(http_server_register_setup_routes(s_ctx.server), TAG, "Failed to register setup routes");
