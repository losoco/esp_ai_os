...
const App: Component = () => {
  const startedWithExplicitHash = hasExplicitHashRoute();
  ...
  let hashSyncEnabled = startedWithExplicitHash;
  ...
  const bootstrap = async () => {
    ...
    try {
      await ensureConfigGroups(['wifi']);
      const wifiConfigured = (appConfig().wifi_ssid ?? '').trim() !== '';
      if (!startedWithExplicitHash && !wifiConfigured) {
        setCurrentTab('start');
      }
    } catch {
      /* keep errors already reported by the initial bootstrap tasks */
    }
  };
